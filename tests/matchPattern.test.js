...
t.test('should match pattern', t => {
    ...
    let matches = [...boardmatcher.matchPattern(data.unfinished, [14, 2], pattern)]
    t.deepEqual(matches.map(x => x.symmetryIndex).sort(), [3, 7])
    t.deepEqual(matches[0].anchors, [[16, 2], [14, 2]])
    t.deepEqual(matches[0].vertices, [[15, 3], [14, 3], [15, 2]])
